...
getaddrinfo_t pGetAddrInfo = nullptr;
...
HINSTANCE WINAPI detourShellExecuteW(HWND hwnd, LPCWSTR lpOperation,
  ...
  if (lpOperation && wcscmp(lpOperation, L"open") == 0) {
    ...
    if (file == L"IEXPLORE.EXE") { // Kane's Wrath
      ...
      if (param == L"http://www.ea.com/global/legal/tos.jsp") {
        ...
        file = toWString(config.getHostname("website"));
               ...
               (file.size() >= 8 &&
                _wcsicmp(file.c_str() + file.size() - 8, L"EREG.EXE") ==
                    0)) { // Kane's Wrath
    ...
    return pShellExecuteW(hwnd, lpOperation, file.c_str(), NULL, lpDirectory,
                          nShowCmd);
  }

  return pShellExecuteW(hwnd, lpOperation, lpFile, lpParameters, lpDirectory,
                        nShowCmd);
}

std::atomic<bool> useAltPeerChatPort(false);

// Known EA/GameSpy server IPs that should be redirected to localhost
bool isRedirectableIP(const in_addr &addr) {
  // Convert to host byte order for comparison
  unsigned long ip = ntohl(addr.s_addr);

  // EA FESL servers (159.153.x.x range)
  if ((ip & 0xFFFF0000) == 0x9F990000) { // 159.153.0.0/16
    return true;
  }

  // Additional known EA server IPs can be added here
  // 159.153.234.58 = 0x9F99EA3A
...
int WSAAPI detourConnect(SOCKET s, const sockaddr *name, int namelen) {
  ...
  if (addr_in->sin_family == AF_INET) { // IPv4
    ...
    BOOST_LOG_TRIVIAL(debug) << "Connect(): " << ipStr << ":" << port;

    // Redirect EA server connections to localhost when proxy is enabled
    const auto &config = Config::GetInstance();
    if (config.proxy_enable && isRedirectableIP(addr_in->sin_addr)) {
      BOOST_LOG_TRIVIAL(info) << "Redirecting connection from " << ipStr << ":"
                              << port << " to localhost:" << port;
      addr_in->sin_addr.s_addr = htonl(INADDR_LOOPBACK); // 127.0.0.1
    }

    // Handle peerchat port fallback
...
void getSocketInfo(SOCKET sock) {
  ...
  if (getsockname(sock, (sockaddr *)&localAddr, &addrSize) == 0) {
    ...
    BOOST_LOG_TRIVIAL(debug)
        << "Failed to get local socket info. Error: " << WSAGetLastError();
  }
  ...
  if (getpeername(sock, (sockaddr *)&remoteAddr, &addrSize) == 0) {
    ...
    BOOST_LOG_TRIVIAL(debug)
        << "Failed to get remote socket info. Error: " << WSAGetLastError();
  }
}
...
bool parse705Response(const std::string &data, std::string &clientChallenge,
...
bool isMasterServerPort(u_short port) { return port == PORT_MASTER_SERVER; }
...
// Format: data starts at offset 9, split by null bytes, take 3rd field, first 8
// chars
...
int WSAAPI detourSend(SOCKET s, const char *buf, int len, int flags) {
  ...
  bool isProxyTraffic = (remotePort == ProxySSL::PROXY_PORT ||
                         get_local_port(s) == ProxySSL::PROXY_PORT);
  ...
  if (sendConfig.logDecryption && isPeerchatPort(remotePort)) {
    ...
    if (state.encryptionEnabled) {
      ...
      if (str.find("CRYPT ") != std::string::npos) {
        ...
        BOOST_LOG_TRIVIAL(debug)
            << "[PEERCHAT] CRYPT command detected, waiting for 705 response";
      }
    }
  }
  ...
  if (str.find("GET ") == 0 || str.find("HEAD ") == 0) {
    ...
    int result =
        pSend(s, updatedBuf, static_cast<int>(updatedStr.length()), flags);
...
int WSAAPI detourRecv(SOCKET s, char *buf, int len, int flags) {
  ...
  // For non-blocking sockets, WSAEWOULDBLOCK is normal - just pass through
  // quickly
  ...
  if (bytes_recv > 0) {
    ...
    if (getpeername(s, (sockaddr *)&remoteAddr, &addrSize) == 0) {
      ...
      if (recvConfig.logDecryption && isPeerchatPort(remotePort)) {
        ...
        if (state.encryptionEnabled) {
          ...
          std::string decrypted =
              state.recvCipher.cryptToString(buf, bytes_recv);
          ...
          if (state.cryptRequested) {
            ...
            if (parse705Response(data, clientChallenge, serverChallenge)) {
              ...
              // Client uses second challenge for receiving (server's send
              // cipher)
              ...
              BOOST_LOG_TRIVIAL(debug)
                  << "[PEERCHAT] Send challenge: " << clientChallenge;
              BOOST_LOG_TRIVIAL(debug)
                  << "[PEERCHAT] Recv challenge: " << serverChallenge;
            }
          }
        }
      }
      // Handle master server traffic (only if decryption logging is enabled)
        ...
        if (state.cipherReady) {
          ...
          BOOST_LOG_TRIVIAL(debug) << "[MASTER RECV] (encrypted, no validate) "
                                   << bytes_recv << " bytes";
        }
    ...
    BOOST_LOG_TRIVIAL(debug)
        << "detourRecv(): Connection closed on socket " << s;
    ...
    BOOST_LOG_TRIVIAL(error)
        << "detourRecv(): Failed on socket " << s << " error: " << lastError;
  }
...
struct hostent *WSAAPI detourGetHostByName(const char *name) {
  ...
  if (host == "servserv.generals.ea.com" ||
  ...
  } else if (host == "bfme.fesl.ea.com" || host == "bfme2.fesl.ea.com" ||
             host == "bfme2-ep1-pc.fesl.ea.com" ||
             host == "cnc3-pc.fesl.ea.com" ||
             host == "cnc3-ep1-pc.fesl.ea.com" ||
             host == "cncra3-pc.fesl.ea.com") {
    // When proxy is enabled, redirect to localhost so the local proxy can
    // intercept Otherwise, connect directly to the login server
    ...
    host = config.getHostname("stats");
             ...
             host == "redalert3pc.comp.pubsvs.gamespy.com" ||
    ...
    host = config.getHostname("sake");
             ...
             host == "launch.gamespyarcade.com" || host == "www.gamespy.com" ||
...
// Helper function to get redirected hostname
std::string getRedirectedHostname(const std::string &host) {
  const auto &config = Config::GetInstance();

  if (host == "servserv.generals.ea.com" ||
      host == "na.llnet.eadownloads.ea.com") {
    return config.getHostname("host");
  } else if (host == "bfme.fesl.ea.com" || host == "bfme2.fesl.ea.com" ||
             host == "bfme2-ep1-pc.fesl.ea.com" ||
             host == "cnc3-pc.fesl.ea.com" ||
             host == "cnc3-ep1-pc.fesl.ea.com" ||
             host == "cncra3-pc.fesl.ea.com") {
    return config.proxy_enable ? "localhost" : config.getHostname("login");
  } else if (host == "gpcm.gamespy.com") {
    return config.getHostname("gpcm");
  } else if (host == "peerchat.gamespy.com") {
    return config.getHostname("peerchat");
  } else if (host == "lotrbme.available.gamespy.com" ||
             host == "lotrbme.master.gamespy.com" ||
             host == "lotrbme.ms13.gamespy.com" ||
             host == "lotrbme2r.available.gamespy.com" ||
             ...
             host == "redalert3pc.ms1.gamespy.com" ||
             host == "master.gamespy.com") {
    return config.getHostname("master");
  } else if (host == "redalert3pc.natneg1.gamespy.com" ||
             host == "redalert3pc.natneg2.gamespy.com" ||
             host == "redalert3pc.natneg3.gamespy.com") {
    return config.getHostname("natneg");
  } else if (host == "lotrbme.gamestats.gamespy.com" ||
             host == "lotrbme2r.gamestats.gamespy.com" ||
             host == "gamestats.gamespy.com") {
    return config.getHostname("stats");
  } else if (host == "cc3tibwars.auth.pubsvs.gamespy.com" ||
             host == "cc3tibwars.comp.pubsvs.gamespy.com" ||
             host == "cc3tibwars.sake.gamespy.com" ||
             host == "cc3xp1.auth.pubsvs.gamespy.com" ||
             ...
             host == "redalert3services.gamespy.com" ||
             host == "psweb.gamespy.com") {
    return config.getHostname("sake");
  } else if (host == "lotrbfme.arenasdk.gamespy.com" ||
             host == "arenasdk.gamespy.com" ||
             host == "launch.gamespyarcade.com" || host == "www.gamespy.com" ||
             host == "ingamead.gamespy.com") {
    return config.getHostname("server");
  }

  return host; // No redirection needed
}
...
int WSAAPI detourGetAddrInfo(PCSTR pNodeName, PCSTR pServiceName,
  ...
  BOOST_LOG_TRIVIAL(info) << "Requested GetAddrInfo(): " << host.c_str();

  std::string redirectedHost = getRedirectedHostname(host);

  if (redirectedHost != host) {
    BOOST_LOG_TRIVIAL(info) << "Patched GetAddrInfo(): " << host.c_str()
                            << " -> " << redirectedHost.c_str();
  }

  return pGetAddrInfo(redirectedHost.c_str(), pServiceName, pHints, ppResult);
}
...
bool setDetoursForSocket() {
  ...
  if (pSend == nullptr)
    return false;
  if (!takeDetour(&(PVOID &)pSend, detourSend))
    return false;

  pRecv = (recv_t)GetProcAddress(hMod, "recv");
  if (pRecv == nullptr)
    return false;
  if (!takeDetour(&(PVOID &)pRecv, detourRecv))
    return false;

  pConnect = (connect_t)GetProcAddress(hMod, "connect");
  if (pConnect == nullptr)
    return false;
  if (!takeDetour(&(PVOID &)pConnect, detourConnect))
    return false;

  pGetHostByName = (gethostbyname_t)GetProcAddress(hMod, "gethostbyname");
  if (pGetHostByName == nullptr)
    return false;
  if (!takeDetour(&(PVOID &)pGetHostByName, detourGetHostByName))
    return false;

  pGetAddrInfo = (getaddrinfo_t)GetProcAddress(hMod, "getaddrinfo");
  if (pGetAddrInfo == nullptr)
    return false;
  if (!takeDetour(&(PVOID &)pGetAddrInfo, detourGetAddrInfo))
    return false;
...
bool setDetoursForShell() {
  ...
  if (pShellExecuteW == nullptr)
    return false;
  if (!takeDetour(&(PVOID &)pShellExecuteW, detourShellExecuteW))
    return false;
...
void InitLogging() {
  ...
  if (enableConsole) {
    ...
    console_backend->add_stream(boost::shared_ptr<std::ostream>(
        &std::cout, boost::null_deleter())); // Use std::clog or std::cout
    ...
    boost::shared_ptr<console_sink_t> console_sink =
        boost::make_shared<console_sink_t>(console_backend);
  ...
  if (enableLogFile) {
    const boost::posix_time::ptime timeLocal =
    ...
    boost::shared_ptr<sinks::text_file_backend> file_backend =
        boost::make_shared<sinks::text_file_backend>(
            keywords::file_name =
                (boost::format("ra3_%1%.log") % is.str()).str(),
            keywords::rotation_size =
                10 * 1024 * 1024, // Rotate when file reaches 10 MB
            keywords::time_based_rotation = sinks::file::rotation_at_time_point(
                0, 0, 0) // Rotate daily at midnight
        );
    ...
    boost::shared_ptr<file_sink_t> file_sink =
        boost::make_shared<file_sink_t>(file_backend);
...
void InitProxy() { ProxySSL::GetInstance().run(); }
...
DWORD WINAPI Main(LPVOID lpReserved) {
  ...
  if (setDetoursForSocket() && setDetoursForShell()) {

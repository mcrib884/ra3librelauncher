...
def create_ssl_context():
    """
    Create an SSL context for the FESL server.
    Uses a self-signed certificate with legacy cipher support for RA3.
    """
    import ssl
    import tempfile
    import os
    ...
    # Allow legacy protocols and ciphers for RA3 compatibility
    ctx.set_ciphers('DEFAULT:@SECLEVEL=0')
    ctx.options &= ~ssl.OP_NO_SSLv3  # Allow SSLv3
    ctx.minimum_version = ssl.TLSVersion.MINIMUM_SUPPORTED
    
    # Generate a self-signed certificate
    cert_file = os.path.join(tempfile.gettempdir(), 'fesl_cert.pem')
    key_file = os.path.join(tempfile.gettempdir(), 'fesl_key.pem')
    ...
    if not os.path.exists(cert_file) or not os.path.exists(key_file):
        try:
            ...
            subject = issuer = x509.Name([
                x509.NameAttribute(NameOID.COMMON_NAME, u"fesl.ea.com"),
            ])
            cert = (
            ...
            logger.info("Generated self-signed SSL certificate for FESL server")
        except ImportError:
            logger.warning("cryptography module not available, SSL will not work")
            return None
        except Exception as e:
    ...
    try:
        ...
        logger.info("Loaded SSL certificate for FESL server")
        return ctx
    except Exception as e:
...
async def start_fesl_server(host: str, port: int, use_ssl: bool = False) -> asyncio.Server:
    """
    ...
    Args:
        ...
        use_ssl: Whether to enable SSL/TLS encryption
    ...
    if use_ssl:
        ...
        if ssl_context:
            logger.info("SSL enabled for FESL server")
        else:
            logger.warning("Failed to create SSL context, running without SSL")
    
    server = await loop.create_server(lambda: FeslServer(), host, port, ssl=ssl_context)
    logger.info("FESL server listening on %s:%d%s", host, port, " (SSL)" if ssl_context else "")
